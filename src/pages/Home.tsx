...
export default function Home() {
  ...
  return <main className="home-luxury bg-[#f6f9fd] text-[#14253f]">
    ...
    <section className="relative overflow-hidden bg-[#ddecfb]">
      ...
      <div className="relative mx-auto grid min-h-[590px] max-w-7xl items-center gap-10 px-5 pb-36 pt-32 sm:px-8 lg:grid-cols-[.9fr_1.1fr] lg:px-12">
        ...
        <div className="relative hidden min-h-[420px] lg:block"><div className="absolute right-0 top-1/2 h-[390px] w-[520px] -translate-y-1/2 rotate-2 overflow-hidden rounded-[2.5rem] border-8 border-white/80 shadow-2xl"><img src={current.image} alt="Travel and event inspiration" width="900" height="650" className="h-full w-full object-cover" /></div></div>
      </div>
      <div className="absolute bottom-0 left-1/2 w-[calc(100%-2.5rem)] max-w-6xl -translate-x-1/2 translate-y-1/2 rounded-2xl border border-[#d8e4ef] bg-white p-2 shadow-2xl shadow-[#16375c]/15"><div className="flex gap-1 rounded-xl bg-[#f1f6fb] p-1">{(['tickets','stays','trip'] as const).map((item) => <button key={item} type="button" onClick={() => setPlanner(item)} className={`rounded-lg px-5 py-3 text-sm font-bold capitalize transition ${planner === item ? 'bg-white text-[#1267c4] shadow-sm' : 'text-[#657890]'}`}>{item === 'trip' ? 'Plan my trip' : item}</button>)}</div><div className="grid gap-2 p-3 sm:grid-cols-[1fr_1fr_1fr_auto]"><div className="rounded-xl border border-[#e2eaf2] px-4 py-3"><span className="block text-xs text-[#8b9aae]">{planner === 'tickets' ? 'What are you seeing?' : 'Where are you going?'}</span><span className="font-semibold text-[#24364d]">Search event or destination</span></div><div className="rounded-xl border border-[#e2eaf2] px-4 py-3"><span className="block text-xs text-[#8b9aae]">Dates</span><span className="font-semibold text-[#24364d]">Choose your dates</span></div><div className="rounded-xl border border-[#e2eaf2] px-4 py-3"><span className="block text-xs text-[#8b9aae]">Travellers</span><span className="font-semibold text-[#24364d]">2 travellers</span></div><Link to={planner === 'tickets' ? '/tickets' : '/listings'} className="flex items-center justify-center rounded-xl bg-[#1267c4] px-6 text-white hover:bg-[#0d56a5]"><Search className="h-5 w-5" /></Link></div></div>
    </section>
