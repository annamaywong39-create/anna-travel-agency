      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="flex items-center gap-4 mb-2">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#DB8293] to-[#C49B55] flex items-center justify-center text-2xl font-bold text-[#14253F]">
              {user.firstName.charAt(0)}{user.lastName.charAt(0)}
            </div>
            <div>
              <h1 className="text-3xl font-black text-[#14253F]">
                Welcome, {user.firstName}!
              </h1>
              <p className="text-[#687A90]">{user.email}</p>
            </div>
          </div>
          {user.role === 'admin' && (
        ...
        <div className="flex gap-2 mb-8 border-b border-[#D8E5F0] pb-4">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all ${
                activeTab === tab.id
                  ...
                  : 'text-[#687A90] hover:text-[#14253F] hover:bg-[#F7FAFD]'
              }`}
            >
        ...
        {activeTab === 'bookings' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <h2 className="text-xl font-bold text-[#14253F] mb-4">
              Your Bookings ({userBookings.length})
            </h2>

            {userBookings.length === 0 ? (
              <Card3D>
                <div className="p-12 text-center bg-white rounded-2xl border border-[#D8E5F0]">
                  <div className="text-5xl mb-4">📅</div>
                  <h3 className="text-xl font-bold text-[#14253F] mb-2">No bookings yet</h3>
                  <p className="text-[#687A90] mb-6">Start planning your next adventure!</p>
                  <Link
                    to="/listings"
                    className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-[#DB8293] to-[#C49B55] text-[#14253F] font-bold hover:scale-105 transition-all"
                  >
              ...
              <div className="space-y-4">
                {userBookings.map((booking) => {
                  ...
                  return (
                    ...
                    >
                      <Card3D>
                        <div className="p-5 flex flex-col md:flex-row gap-4 bg-white rounded-2xl border border-[#D8E5F0]">
                          {listing && (
                            <img
                          ...
                          <div className="flex-1">
                            <div className="flex items-start justify-between gap-2 mb-2">
                              <div>
                                <h3 className="text-[#14253F] font-bold">{listing?.title || 'Property'}</h3>
                                <p className="text-[#687A90] text-sm flex items-center gap-1">
                            ...
                            <div className="flex flex-wrap gap-4 text-sm text-[#687A90] mb-3">
            ...
            <Card3D>
              <div className="p-6 bg-white rounded-2xl border border-[#D8E5F0]">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-bold text-[#14253F]">Profile Information</h2>
                  <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={field.key}>
                      <label className="text-sm text-[#687A90] mb-1 block">{field.label}</label>
                      {editMode ? (
                        <input
                          type="text"
                          value={profileData[field.key as keyof typeof profileData]}
                          onChange={(e) => setProfileData(prev => ({ ...prev, [field.key]: e.target.value }))}
                          className="w-full px-4 py-3 rounded-xl bg-[#F7FAFD] border border-[#D8E5F0] text-[#14253F] focus:outline-none focus:border-[#DB8293] transition-all"
                        />
                      ) : (
                        <p className="px-4 py-3 rounded-xl bg-[#F7FAFD] text-[#14253F]">
                ...
                <div className="mt-6 pt-6 border-t border-[#D8E5F0]">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-[#14253F] font-medium">Email Address</p>
                      <p className="text-[#687A90] text-sm">{user.email}</p>
                    </div>
                    <span className="text-xs text-green-400 bg-green-500/10 px-2 py-1 rounded">Verified</span>
                  </div>
                </div>

                <div className="mt-4 pt-4 border-t border-[#D8E5F0]">
        ...
        {activeTab === 'settings' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
            <Card3D>
              <div className="p-6 bg-white rounded-2xl border border-[#D8E5F0]">
                <h3 className="text-[#14253F] font-bold mb-4">Notifications</h3>
                  ...
                  {[
                    ...
                    <label key={item.label} className="flex items-center justify-between cursor-pointer">
                      ...
                      <input
                        ...
                        className="w-5 h-5 rounded border-gray-600 bg-[#F7FAFD] text-[#DB8293] focus:ring-[#DB8293]/20"
                      />
                    </label>
                  ))}
                </div>
            ...
            <Card3D>
              <div className="p-6 bg-white rounded-2xl border border-[#D8E5F0]">
                <h3 className="text-[#14253F] font-bold mb-4">Account Actions</h3>
                <div className="space-y-3">
                  <button className="w-full text-left px-4 py-3 rounded-xl bg-[#F7FAFD] text-gray-300 hover:bg-white transition-all flex items-center justify-between">
