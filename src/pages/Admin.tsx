...
type Tab = 'overview' | 'listings' | 'bookings' | 'users' | 'matchprices';

export default function Admin() {
  const { user } = useAuth();
  const { listings, bookings, deleteListing, updateBooking, isDemo, fetchAllUsers, fetchMatches, updateMatchPrices } = useData();
  ...
  const saveMatchPrices = async () => {
    ...
    try {
      for (const matchId of Object.keys(priceChanges)) {
        const updates = priceChanges[matchId];
        await updateMatchPrices(matchId, updates);
      }
      await loadMatches();
                ...
                <Card3D key={listing.id}>
                  <div className="relative">
                    ...
                    <div className="absolute top-3 right-3 flex gap-2">
                      <Link to={`/admin/listing/${listing.id}`} className="w-8 h-8 rounded-lg bg-blue-500/80 backdrop-blur-sm flex items-center justify-center text-white hover:bg-blue-500 transition-all"><Edit2 className="w-4 h-4" /></Link>
                      <button onClick={() => { if (confirm('Delete this listing?')) deleteListing(listing.id); }} className="w-8 h-8 rounded-lg bg-red-500/80 backdrop-blur-sm flex items-center justify-center text-white hover:bg-red-500 transition-all"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </div>
                  <div className="p-4">
                        ...
                        <tr key={booking.id} className="border-b border-white/5 hover:bg-white/5">
                          ...
                          <td className="p-4">
                            ...
                            {emailStatus && emailStatus.id === booking.id && (
                              <span className={`ml-2 text-xs ${emailStatus.status === 'sending' ? 'text-yellow-400' : emailStatus.status === 'sent' ? 'text-green-400' : 'text-red-400'}`}>
                                {emailStatus.status === 'sending' ? '📧 Sending...' : emailStatus.status === 'sent' ? '✅ Email sent!' : '❌ Failed'}
                              </span>
                            )}
                          </td>
            ...
            <Card3D>
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={loadUsers} className="px-4 py-2 rounded-lg bg-white/5 text-gray-400 hover:bg-white/10 transition-all text-sm flex items-center gap-2">
                    <span>🔄</span> Refresh
                  </button>
                </div>
                {loadingUsers ? (
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={loadMatches} className="px-4 py-2 rounded-lg bg-white/5 text-gray-400 hover:bg-white/10 transition-all text-sm flex items-center gap-2">
                      <RefreshCw className="w-4 h-4" /> Refresh
                    </button>
                    {Object.keys(priceChanges).length > 0 && (
                      <button onClick={saveMatchPrices} disabled={savingPrices} className="px-4 py-2 rounded-lg bg-gradient-to-r from-amber-500 to-red-500 text-white hover:scale-105 transition-all text-sm flex items-center gap-2 disabled:opacity-50">
                ...
                {loadingMatches ? (
                  ...
                  <div className="py-12 text-center text-gray-400">No matches found from July 1 onward.</div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-white/10">
                          ...
                          <th className="text-left p-4 text-gray-400 text-sm font-medium">Category 1</th>
                          <th className="text-left p-4 text-gray-400 text-sm font-medium">Category 2</th>
                          <th className="text-left p-4 text-gray-400 text-sm font-medium">Category 3</th>
                          <th className="text-left p-4 text-gray-400 text-sm font-medium">Category 4</th>
                          <th className="text-left p-4 text-gray-400 text-sm font-medium">Supporter Entry</th>
                        </tr>
                      </thead>
                      <tbody>
                        {matches.map((match) => {
