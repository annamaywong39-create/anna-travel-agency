...
export default function Tickets() {
  ...
  const handleBuy = (item: DisplayItem) => {
    ...
    setSelectedCategory(item.type === 'match' ? 'category_1' : (item.tickets[0]?.id || ''));
    setQuantity(1);
  ...
  return (
    <main className="pt-24 pb-20 min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          <div className="py-20 text-center"><div className="inline-block animate-spin rounded-full h-10 w-10 border-4 border-amber-500 border-t-transparent" /><p className="text-gray-400 mt-4">Loading...</p></div>
        ) : items.length === 0 ? (
          <div className="py-20 text-center"><div className="text-5xl mb-4">🎟️</div><p className="text-gray-400">No tickets available.</p></div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {items.map((item) => {
              ...
              const availCats = isMatch
                ...
                : (item as EventItem).tickets.map(t => ({ id: t.id, name: t.category_name, price: t.price, icon: '🎫', color: 'from-purple-500 to-pink-400' }));
              return (
                  ...
                  <Card3D glowColor={isFinal ? 'rgba(245, 158, 11, 0.2)' : 'rgba(245, 158, 11, 0.1)'}>
                    <div className={`p-6 ${isFinal ? 'border-b-2 border-amber-500/30' : ''}`}>
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className={`px-3 py-1 rounded-full text-xs border ${item.status === 'live' ? 'bg-red-500/20 text-red-400 border-red-500/30 animate-pulse' : item.status === 'finished' ? 'bg-gray-500/20 text-gray-400 border-gray-500/30' : 'bg-green-500/20 text-green-400 border-green-500/30'}`}>
                          {item.status}
                        </span>
                      </div>
                      <div className="mb-3">
                      ...
                      <div className="mt-4 grid grid-cols-2 md:grid-cols-3 gap-2">
                        {availCats.map((cat: any) => (
                          <div key={cat.id} className={`text-center p-2 rounded-lg bg-gradient-to-br ${cat.color} bg-opacity-10 border border-white/10`}>
                            ...
                            <p className="text-white font-bold text-sm">${cat.price}</p>
                            <p className="text-gray-400 text-[10px]">{cat.name}</p>
                          </div>
                        ))}
                      </div>
                      <button onClick={() => handleBuy(item)} className={`mt-4 w-full py-3 rounded-xl font-bold hover:scale-105 transition-all shadow-lg ${item.status === 'finished' ? 'bg-gray-600/50 text-gray-400 cursor-not-allowed' : 'bg-gradient-to-r from-amber-500 to-red-500 text-white shadow-amber-500/25'}`} disabled={item.status === 'finished'}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={closeModal}>
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-[#14142a] rounded-2xl border border-white/10 max-w-md w-full p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-2xl font-bold text-white">{selectedItem.type === 'match' ? `${(selectedItem as MatchTicket).home_team} vs ${(selectedItem as MatchTicket).away_team}` : (selectedItem as EventItem).title}</h3>
            ...
            <div className="space-y-3 mb-6">
              ...
              {selectedItem.type === 'match' ? (
                  ...
                  return (
                    <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} className={`w-full flex items-center justify-between p-3 rounded-xl transition-all border ${isSelected ? 'bg-amber-500/20 border-amber-500/40 ring-1 ring-amber-500/20' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left"><p className={`text-sm font-medium ${isSelected ? 'text-amber-300' : 'text-white'}`}>{cat.name}</p><p className="text-gray-500 text-xs">{cat.description}</p></div>
                      </div>
                ...
                (selectedItem as EventItem).tickets.map((t) => {
                  ...
                  return (
                    <button key={t.id} onClick={() => setSelectedCategory(t.id)} className={`w-full flex items-center justify-between p-3 rounded-xl transition-all border ${isSelected ? 'bg-amber-500/20 border-amber-500/40 ring-1 ring-amber-500/20' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>
                      <div className="flex items-center gap-3">
                        <span className="text-xl">🎫</span>
                        <div className="text-left"><p className={`text-sm font-medium ${isSelected ? 'text-amber-300' : 'text-white'}`}>{t.category_name}</p><p className="text-gray-500 text-xs">{t.quantity_available} available</p></div>
                      </div>
            ...
            <button onClick={handlePurchase} className="w-full py-3 rounded-xl bg-gradient-to-r from-amber-500 to-red-500 text-white font-bold hover:scale-105 transition-all shadow-lg shadow-amber-500/25">
              <Ticket className="w-4 h-4 inline mr-2" /> Purchase
            </button>
      ...
      {purchaseSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-[#14142a] rounded-2xl border border-green-500/30 max-w-md w-full p-8 text-center shadow-2xl">
            ...
            <p className="text-gray-400 mb-4">{quantity} ticket{quantity > 1 ? 's' : ''} added to your cart.</p>
