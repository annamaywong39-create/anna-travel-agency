...
import { Calendar, Filter, MapPin, Search, Ticket as TicketIcon, X, ShoppingCart, Hotel, Grid3X3, List } from 'lucide-react';
...
function btsDiscountPercent(ticket: TicketOption) {
  if (Number.isFinite(ticket.discount_percent)) return Math.max(0, Math.min(70, Number(ticket.discount_percent)));
  // Safe fallback for older BTS rows created before discount_percent was added.
...
export default function Tickets() {
  ...
  const addTicket = () => {
    ...
    const discounted = isBtsBaltimoreEvent(selected) || selectedTicket.discount_percent !== undefined;
