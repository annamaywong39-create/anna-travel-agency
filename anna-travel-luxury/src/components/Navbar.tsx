...
export default function Navbar() {
  ...
  return <>
    <header className={`fixed inset-x-0 top-0 z-50 border-b backdrop-blur-xl ${isAdminArea ? 'border-white/10 bg-[#14253F]/95 text-white' : 'border-[#E7F1FC] bg-white/90 text-[#14253f]'}`}>
      <div className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-5 sm:px-8 lg:px-12">
        <Link to="/" className="flex shrink-0 items-center gap-3"><img src="/logo.png" alt="Anna Travel Agency" width="56" height="56" className="h-10 w-10 rounded-xl object-cover" /><span className="hidden sm:block"><span className={`block font-serif text-[18px] leading-none ${isAdminArea ? 'text-white' : 'text-[#14253f]'}`}>Anna</span><span className="mt-0.5 block text-[8px] font-bold uppercase tracking-[.28em] text-[#b08a42]">Travel agency</span></span></Link>
        
        {/* Desktop nav - visible from md up */}
        <nav className="hidden items-center gap-6 md:flex">
          {links.map((link) => <Link key={link.to} to={link.to} className={`text-[13px] font-semibold transition ${location.pathname === link.to ? 'text-[#1267C4]' : isAdminArea ? 'text-white/75 hover:text-white' : 'text-[#536071] hover:text-[#172033]'}`}>{link.label}</Link>)}
        </nav>
        ...
        <div className="hidden items-center gap-2.5 md:flex">
          ...
          {isAuthReady && (user ? <div className="group relative"><button className="flex items-center gap-2 rounded-full border border-[#D8E5F0] px-3 py-2 text-sm font-semibold text-[#536071]"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#172033] text-xs text-[#f3d99a]">{user.firstName?.charAt(0) || 'U'}</span><span className="max-w-[80px] truncate">{user.firstName}</span></button><div className="invisible absolute right-0 top-full mt-2 w-48 rounded-2xl border border-[#D8E5F0] bg-white p-2 opacity-0 shadow-xl transition group-hover:visible group-hover:opacity-100"><Link to="/dashboard" className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm text-[#536071] hover:bg-[#F7FAFD]"><UserRound className="h-4 w-4" />Dashboard</Link>{user.role === 'admin' && <Link to="/admin" className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold text-[#1267C4] hover:bg-[#E7F1FC]"><Shield className="h-4 w-4" />Admin panel</Link>}<button onClick={logout} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm text-red-500 hover:bg-red-50"><LogOut className="h-4 w-4" />Sign out</button></div></div> : <><Link to="/login" className="rounded-full border border-[#D8E5F0] px-4 py-2 text-sm font-semibold text-[#536071] hover:bg-[#F7FAFD]">Login</Link><Link to="/signup" className="rounded-full bg-[#14253F] px-4 py-2 text-sm font-bold text-white hover:bg-black">Sign up</Link></>)}
        </div>

        {/* Mobile menu button */}
        <button onClick={() => setOpen((v) => !v)} aria-label={open ? 'Close menu' : 'Open menu'} className="rounded-full border border-[#D8E5F0] p-2.5 text-[#14253F] md:hidden">{open ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button>
      </div>
    </header>

    {open && <div className="fixed inset-0 z-[100] overflow-y-auto overscroll-contain bg-[#F7FAFD] px-5 pb-10 pt-24 md:hidden">
      <div className="flex flex-col gap-1.5">
        {links.map((link) => <Link key={link.to} to={link.to} className={`rounded-2xl px-5 py-4 text-[18px] font-semibold ${location.pathname===link.to ? 'bg-[#E7F1FC] text-[#1267C4]' : 'text-[#14253F] hover:bg-white'}`}>{link.label}</Link>)}
